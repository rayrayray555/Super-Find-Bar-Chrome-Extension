...
**问题**：实时刷新时，高亮会闪烁（清除后重新创建），影响用户体验。

**解决方案**：使用增量更新机制，只更新变化的高亮，保持不变的不重新创建。

**实现流程**：
```
highlightAll(isAutoRefresh):
  ├─ 手动搜索（isAutoRefresh = false）
  │   ├─ 清除所有高亮（CSS.highlights.clear()）
  │   ├─ 清除注册表（state.currentHighlightIds, state.highlightRegistry）
  │   └─ 重新创建所有高亮
  │
  └─ 自动刷新（isAutoRefresh = true）
      ├─ 生成新的高亮标识集合（newHighlightIds）
      ├─ 比较新旧标识集合
      │   ├─ toDelete: 旧的有，新的没有
      │   └─ toAdd: 新的有，旧的没有
      ├─ 如果有变化：
      ...
      │   │   └─ 更新 CSS.highlights（只更新有变化的颜色组）
      │   ├─ 更新注册表（删除旧的，添加新的）
      │   └─ 更新状态（state.currentHighlightIds = newHighlightIds）
      └─ 如果没有变化：不更新高亮（避免闪烁）
```
...
**关键点**：
- 使用唯一标识（`generateRangeId()`）跟踪每个高亮
- 只更新有变化的颜色组，避免全量清除和重建
- 手动搜索时仍然全量清除（符合用户预期）
- 自动刷新时使用增量更新，避免闪烁
...
**问题**：实时刷新时，高亮和XY轴标记会闪烁（清除后重新创建），影响用户体验。

**解决方案**：使用增量更新机制，只更新变化的部分，保持不变的部分不重新创建。

**实现流程**：
```
搜索完成/自动刷新:
  ├─ 生成唯一标识（generateRangeId）
  │   ├─ 普通文本：基于容器节点、偏移量、颜色
  │   └─ 输入框：基于元素路径、匹配位置、颜色
  │
  ├─ 高亮增量更新（highlightAll）
  ...
  │       ├─ 找出需要删除的高亮（旧的有，新的没有）
  │       ├─ 找出需要添加的高亮（新的有，旧的没有）
  │       ├─ 只更新有变化的颜色组（重新构建该颜色组的所有Range）
  │       └─ 保持不变的颜色组不更新
  │
  └─ XY轴标记增量更新（drawTickBarImmediate）
      ├─ 手动搜索：清除所有标记，重新创建
      └─ 自动刷新：
          ├─ 比较新旧标识集合
          ├─ 找出需要删除的标记（旧的有，新的没有）
...
**关键点**：
- 使用唯一标识（Range ID）来跟踪每个高亮/标记
- 只更新有变化的颜色组/标记，避免全量清除和重建
- 手动搜索时仍然全量清除（符合用户预期）
- 自动刷新时使用增量更新，避免闪烁

### 4.7 自动刷新时的隐藏状态检查（重要）

**问题**：切换"包含隐藏选项"后，如果触发自动刷新，隐藏状态的虚线提示会消失。

**解决方案**：在自动刷新模式下，如果保留了当前索引，也需要检查隐藏状态。

**实现流程**：
```
自动刷新完成:
  ├─ 如果保留了当前索引（state.idx >= 0）
  │   ├─ 更新高亮和UI
  │   └─ 延迟150ms检查隐藏状态（checkCurrentResultHiddenState）
  │       ├─ 如果隐藏 → 显示虚线边框和提示
  │       └─ 如果可见 → 移除警告样式
  │
  └─ 如果索引无效或没有结果
...
**关键点**：
- 搜索完成后（手动搜索）：立即检查隐藏状态
- 自动刷新完成后：如果保留了当前索引，也检查隐藏状态
- 使用统一的 `checkCurrentResultHiddenState()` 函数
- 延迟150ms检查，确保高亮更新完成
...
go(dir):
  ...
  ├─ 验证 Range 有效性
  ├─ 清除所有相关定时器（避免冲突）
  ├─ 移除 warn-hidden 类（准备重新检查）
  ├─ 使用 requestAnimationFrame 立即调用 highlightAll()（不使用防抖）
  ├─ 延迟检查隐藏状态（100ms）
  │   ├─ 使用闭包保存 currentRange 引用
  │   ├─ 检查时再次验证 Range 有效性
  │   └─ 如果隐藏 → 显示虚线边框和提示
  └─ 调用 checkAndRefreshAfterSwitch()（1秒后启动监听器）
```

### 5.4 隐藏状态检查机制

**检查时机**：
1. **搜索完成后**：设置第一个结果后，延迟150ms检查
2. **切换结果时**：`go()` 函数中延迟100ms检查
3. **自动刷新后**：如果保留了当前索引，延迟150ms检查

**检查函数**：`checkCurrentResultHiddenState()`
- 获取当前激活的 Range
- 使用 `isVisible(element, false)` 判断是否隐藏
- 如果隐藏：显示虚线边框（`warn-hidden`）和提示消息
- 如果可见：移除警告样式

**关键点**：
- 使用统一的检查函数，确保逻辑一致
- 延迟检查，确保高亮更新完成后再检查
- 使用闭包保存 Range 引用，避免异步检查时引用失效
...
- `currentHighlightIds`: 当前高亮的唯一标识集合（用于增量更新）
- `currentMarkerIds`: 当前标记的唯一标识集合（用于增量更新）
- `highlightRegistry`: 高亮注册表（id -> { range, color }）
- `markerRegistry`: 标记注册表（id -> { element, rangeData, type }）
